# -*- coding: utf-8 -*- 
# @Time : 2025/10/29 10:31 
# @Author : Lingzi Zhao 
# @File : Param-GPPO-TS-浙江永康.py
"""
Min-Max mTSP (~1000 nodes, m=5) — V7 (参数化动作 + 辅助任务)
本版改动：
- 距离：任务-任务来自 distance_matrix.csv；仓库<->任务用欧氏距离
- 路线长度 = 边长之和 + 节点长度之和（road_length.csv；仓库节点长度=0）
- 记录：progress.csv / routes_best.json / ppo_losses.csv / ppo_loss_curve.png
- 整个路径长度 = (出发到第一个任务 + 各任务间的距离 + 最后任务回仓库距离)＋ 任务节点自身长度和
- 基于PPO 强化学习与 GNN 结合的混合启发式算法，用于求解含 1000 个节点、5 个销售员的最小最大多旅行商问题（Min-Max mTSP） ，核心目标是最小化所有销售员路线长度的最大值，同时兼顾路线长度的均衡性。
"""
...
# =================== Config ===================
CSV_PATH     = "芜湖鸠江_task_coordinate.csv"
ROADLEN_CSV  = "road_length.csv"      # 节点自身长度：id,length
DISTM_PATH   = "distance_matrix.csv"  # 任务-任务实际距离矩阵
SEED = 42

M_SALESMEN = 5
LAMBDA_BAL = 0.15  # soft(min-max) 中 std 的权重

# Graph for GNN（仍用欧氏构图）
KNN_K = 12
GNN_HIDDEN = 96
GNN_LAYERS = 2
...
# ---- Lexicographic reward & stabilization ----
...
# Early operator diversification
RR_EARLY_STEPS = 300

# Periodic intensify & diversify
INTENSIFY_EVERY  = 25
DIVERSIFY_EVERY  = 120
LIGHT_PERTURB_FRAC = 0.03

# Operator credit bias
...
def build_hybrid_distance(coords: np.ndarray,
    ...
    """
    混合距离：
    - 仓库 index=0；其余 1..N-1 为任务
    - 任务-任务：distance_matrix.csv（按原始 id 对齐）
    - 仓库<->任务：欧氏
    """
    N = len(coords)
...
def tour_length(route: List[int], dist: np.ndarray, node_len: np.ndarray) -> float:
    ...
    node_sum = float(sum(node_len[v] for v in route[1:-1]))  # 不含仓库0
...
def angle_sweep_partition(coords: np.ndarray, m: int, theta0: float = 0.0) -> List[np.ndarray]:
    ...
    n = len(order); base = n // m; rem = n % m
    clusters = []; start = 0
    for k in range(m):
        sz = base + (1 if k < rem else 0)
        idx = order[start:start+sz]; start += sz
...
def build_init(coords: np.ndarray, m: int, dist_all: np.ndarray, node_len: np.ndarray) -> List[List[int]]:
    ...
    for s in range(INIT_SWEEP_STARTS):
        ...
        routes = []
        for sub in clusters:
            dsub = submatrix(dist_all, sub)  # 初始化用“实际边距离”
            r = [0]; unv = set(range(1, len(sub))); cur = 0
            while unv:
                nxt = min(unv, key=lambda j: dsub[cur,j])
                r.append(nxt); unv.remove(nxt); cur = nxt
            r.append(0)
...
def try_inter_relocate(routes: List[List[int]], dist: np.ndarray, node_len: np.ndarray) -> List[List[int]]:
    ...
    long_r = routes[ridx]
    if len(long_r) <= 3: return routes
    cand_idx = top_contrib_nodes(long_r, dist, TOP_CAND_NODES)
    best_soft = soft_obj_by_lengths(Ls); best = None
    for i in cand_idx:
        node = long_r[i]
        ...
        for t in range(len(routes)):
            ...
            for pos in positions:
                ...
                if s < best_soft - 1e-12:
                    best_soft = s; best = (ridx, i, t, pos)
    if best is None: return routes
    ridx, i, t, pos = best
...
def try_inter_swap(routes: List[List[int]], dist: np.ndarray, node_len: np.ndarray) -> List[List[int]]:
    ...
    best = None; best_soft = soft_obj_by_lengths(Ls)
    for i in cand_i:
        nodeA = long_r[i]
        for t in range(len(routes)):
            if t == ridx: continue
            r2 = routes[t]
            if len(r2) <= 3: continue
...
def ruin_recreate(routes: List[List[int]], dist: np.ndarray, coords: np.ndarray,
    ...
    all_nodes = []
    for r in routes: all_nodes.extend(r[1:-1])
    n = len(all_nodes)
    ruin_n = max(2, int(frac * n))
    Ls = lengths_all(routes, dist, node_len)
    ridx = int(np.argmax(Ls)); long_r = routes[ridx]
...
def targeted_kick(routes: List[List[int]], dist: np.ndarray, step_idx: int) -> List[List[int]]:
    ...
    tgt_list = list(range(len(new_routes))); tgt_list.remove(ridx)
    random.shuffle(tgt_list)
    best_pos = None; best_t = None; best_cost = float('inf')
    for t in tgt_list:
        r2 = new_routes[t]
...
class PolicyValueNet(nn.Module):
    def __init__(self, node_feat_dim, gnn_hid, n_ops, stat_dim):
        ...
        self.src_node_head = nn.Linear(gnn_hid, 1)     # (N,1)
        self.tgt_route_head = nn.Linear(gnn_hid, 1)    # (R,1)
...
def build_knn_graph(coords: np.ndarray, k: int) -> np.ndarray:
    N = len(coords); dist = euclidian(coords)
    edges = []
    for i in range(N):
...
def build_node_features(coords: np.ndarray, routes: List[List[int]]) -> torch.Tensor:
    """
    Node features: [x, y, dist0, normx, normy, route_onehot(m), deg/2]
    """
    N = len(coords)
    ...
    feat = np.concatenate([xy, dist0, norm_xy, route_oh, (deg/2.0).reshape(-1,1)], axis=1)
    feat[0] = 0.0
...
def build_stat_vec(routes: List[List[int]], dist_all: np.ndarray, node_len: np.ndarray, t: int, best_soft: float) -> torch.Tensor:
    ...
    cur_max = float(arr.max()); cur_mean = float(arr.mean()); cur_std = float(arr.std()+1e-9)
...
class PPOBuffer:
    def __init__(self):
        ...
        self.actions = []          # op index
        self.logprobs = []         # total (op + param)
        self.op_logprobs = []      # 诊断
        self.param_logprobs = []   # 诊断
        self.values = []
        self.rewards = []
        self.dones = []
        self.aux_targets = []      # [dmax_norm, dstd_norm]
        self.param_infos = []      # 参数化选择（或 None）
...
class PPOAgent:
    def __init__(self, node_feat_dim, stat_dim, n_ops):
        ...
        self.aux_coef = 0.05  # 辅助损失权重

    @staticmethod
    def _mask_to_logits(mask_bool: torch.Tensor, fill: float = -1e9):
        return torch.where(mask_bool > 0, torch.zeros_like(mask_bool, dtype=torch.float32),
    ...
    def _pick_param_inter_relocate(self, routes, node_h):
        ...
        if not idx_src_nodes: return None
        ...
        for pos in pos_idx_list:
            pre = r_tgt[pos-1]; post = r_tgt[pos]
    ...
    def _pick_param_inter_swap(self, routes, node_h):
        ...
        if int(pickB.item()) not in r_tgt: return None
    ...
    def select_action(self, node_x, edge_index, num_nodes, stat_vec, op_bias=None, routes=None):
        ...
        return int(a_op.item()), total_logp.detach(), value.view(1).detach(), param, aux.detach(), logp_op.detach().view(1), param_logp.detach().view(1)
    ...
    def ppo_update(self, buffer, coords, edge_index, dist_all, node_len, best_soft, update_steps, start_t, op_bias=None):
        N = len(coords); num_nodes = torch.tensor(N, device=DEVICE)
        ...
        with torch.no_grad():
            ...
            op_logits, last_value, last_aux, _ = self.net(last_node_x, edge_index, num_nodes, last_stat)
            last_value = last_value.view(1)
        values = torch.cat([old_values, last_value], dim=0)

        adv = torch.zeros_like(rewards, device=DEVICE); gae = 0.0
        ...
        for epoch in range(PPO_EPOCHS):
            ...
            for start in range(0, len(idxs), PPO_MINI_BATCH):
                ...
                routes_mb = [buffer.routes_snapshots[i] for i in mb_idx]
                act_mb    = actions[mb_idx]
                old_total_logp_mb = old_total_logp[mb_idx]
                adv_mb    = adv[mb_idx]
                # 这里 returns 未标准化；也可按 V14 做归一化
                ...
                for i, (nx, st, a) in enumerate(zip(node_x_mb, stat_mb, act_mb.tolist())):
                    ...
                    op_logits, v_tmp, aux_pred, _ = self.net(nx, edge_index, num_nodes, st)
...
# ========= Environment with param & fallback =========
def apply_operator(op: str, routes: List[List[int]], dist_all: np.ndarray,
    ...
    if op == "intra_2opt":
        ...
        new_routes = [list(r) for r in routes]; new_routes[ridx] = new_r
    elif op == "intra_oropt":
        Ls_edges = [sum(dist_all[r[i], r[i+1]] for i in range(len(r)-1)) for r in routes]
        ridx = int(np.argmax(Ls_edges))
        r = routes[ridx]
        for _ in range(12):
            ...
            if g <= 1e-12: break
            r = r2
        new_routes = [list(x) for x in routes]; new_routes[ridx] = r
...
def apply_operator_with_param(op, routes, dist_all, coords, node_len, step_idx, param) -> Optional[List[List[int]]]:
    ...
    try:
        ...
        if op == "inter_relocate" and param.get("op") == "inter_relocate":
            ...
            if rs == rt: return None
            if not (0 < i < len(new_routes[rs]) - 1): return None
        ...
        if op == "inter_swap" and param.get("op") == "inter_swap":
            ...
            if rs == rt: return None
            if not (0 < i < len(new_routes[rs]) - 1): return None
            if not (0 < j < len(new_routes[rt]) - 1): return None
...
def main():
    ...
    if os.path.exists(ROADLEN_CSV):
        ...
        print("[WARN] road_length.csv 不存在，所有节点长度视为0。")
    ...
    for idx in range(1, N):
        ...
        ln = None
        if isinstance(oid, int) and oid in node_len_map: ln = node_len_map[oid]
        elif str(oid) in node_len_map: ln = node_len_map[str(oid)]
    ...
    # ---- PPO ----
    node_feat_dim = 5 + M_SALESMEN + 1  # [x,y,dist0,normx,normy] + route OH + deg/2
    stat_dim = 9
    ...
    for t in range(1, TOTAL_STEPS + 1):
        ...
        # 存 buffer
        buffer.add(routes, action_idx, logp_total, value, reward, 0.0,
        ...
        if t % PPO_BATCH == 0 and t > RR_EARLY_STEPS:
            stats = agent.ppo_update(buffer, coords, edge_index, dist_all, node_len,
        ...
        if t % PRINT_EVERY == 0 or t == 1:
            Ls = lengths_all(routes, dist_all, node_len)
            ridx = int(np.argmax(Ls))
            print(f"[{t:5d}/{TOTAL_STEPS}] op={OPS[action_idx]:18s} reward(inst)={inst_reward:+8.4f} "
